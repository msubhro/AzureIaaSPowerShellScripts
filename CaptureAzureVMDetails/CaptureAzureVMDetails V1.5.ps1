...
# This script captures all the necessary information related to Azure VM, and will export those information in a CSV file.

# This script runs on PowerShell AZ Module.

# The script is tested only on Windows VM.
...
$vmdetails= Get-AzVM -ResourceGroupName $ResourceGroup -Name $vm
...
$OSDiskName = (get-azdisk -ResourceGroupName $ResourceGroup -Name $vhd).Name
...
$OSDiskSize = (get-azdisk -ResourceGroupName $ResourceGroup -DiskName $vhd).DiskSizeGB
...
$Nic = Get-AzNetworkInterface -ResourceGroupName $ResourceGroup -Name $NicName1
...
$PublicIP = (Get-AzPublicIpAddress -Name $PublicIP3 -ResourceGroupName $ResourceGroup).IpAddress
...
$NICProperty = Get-AzNetworkInterface -ResourceGroupName $ResourceGroup -Name $NicName1
...
$OSVolumeEncrypted = (Get-AzVMDiskEncryptionStatus -ResourceGroupName $resourcegroup -VMName $vm).OsVolumeEncrypted

$DataVolumeEncrypted = (Get-AzVMDiskEncryptionStatus -ResourceGroupName $resourcegroup -VMName $vm).DataVolumesEncrypted
...
$StorageAccountType= (Get-AzStorageAccount -ResourceGroupName $ResourceGroup -AccountName $StorageAccountName).Sku.Name
...
$DataStorageAccountType= (Get-AzStorageAccount -ResourceGroupName $ResourceGroup -AccountName $DataStorageAccountName).Sku.Name
...
$vmlist= (Get-AzVM -ResourceGroupName $ResourceGroup).Name
